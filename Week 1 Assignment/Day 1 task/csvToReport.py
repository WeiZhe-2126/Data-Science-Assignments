...
for current_day in days_list:
    ...
    for row in sheet.iter_rows():
        for cell in row:
            ...
            if value_to_find_HGain and cell.value == value_to_find_HGain:
                ...
                sum_HDR_HGain_value = sheet.cell(row=next_row_for_HGain, column=cell.column).value = sum_HGain
                ...
                sheet.cell(row=next_row_for_CH_KW, column=cell.column).value = sum_CH_KW
                ...
                sheet.cell(row=next_row_for_CHWP_KW, column=cell.column).value = sum_CHWP_KW
                ...
                sheet.cell(row=next_row_for_CDWP_KW, column=cell.column).value = sum_CDWP_KW
                ...
                sheet.cell(row=next_row_for_CT_KW, column=cell.column).value = sum_CT_KW
                ...
                sum_Saving_kWh_value = sheet.cell(row=next_row_for_Saving_kWh, column=cell.column).value = sum_Saving_kWh
                ...
                sum_HDR_CH_Eff = sheet.cell(row=next_row_for_HDR_CH_Eff, column=cell.column).value = sum_CH_KW/sum_HGain
                ...
                sum_HDR_Plant_Eff = sheet.cell(row=next_row_for_HDR_Plant_Eff, column=cell.column).value = ( ( sum_CH_KW + sum_CHWP_KW + sum_CDWP_KW + sum_CT_KW ) / sum_HGain )
                ...
                saving_percentage_value = sheet.cell(row=next_row_for_Saving_Percantage, column=cell.column).value = (sum_Saving_kWh_value / (sum_HDR_HGain_value * baseline_value)) * 100.00
    ...
    if sum_HGain is not None:
        print(f" - Total Heat Gain (RTh): {sum_HGain}")
    if sum_CH_KW is not None:
        print(f" - Chiller Total Energy (kWh): {sum_CH_KW}")
    if sum_CHWP_KW is not None:
        print(f" - CHWP Total Energy (kWh): {sum_CHWP_KW}")
    if sum_CDWP_KW is not None:
        print(f" - CDWP Total Energy (kWh): {sum_CDWP_KW}")
    if sum_CT_KW is not None:
        print(f" - CT Total Energy (kWh): {sum_CT_KW}")
    if sum_Saving_kWh is not None:
        print(f" - Energy Saving (kWh): {sum_Saving_kWh}")
    if sum_HDR_CH_Eff is not None:
        print(f" - Chiller Efficiency (kW/RT): {sum_HDR_CH_Eff}")
    if sum_HDR_Plant_Eff is not None:
        print(f" - Total Plant Efficiency (kW/RT): {sum_HDR_Plant_Eff}")
    if saving_percentage_value is not None:
        print(f" - Energy Saving (%): {saving_percentage_value}")
